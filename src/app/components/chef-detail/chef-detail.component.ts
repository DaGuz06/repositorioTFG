...
import { ChefRatingComponent } from '../chef-rating/chef-rating.component';

@Component({
    selector: 'app-chef-detail',
    standalone: true,
    imports: [CommonModule, RouterModule, ChefRatingComponent],
...
export class ChefDetailComponent implements OnInit {
    ...
    onRatingSubmitted(event: { rating: number; comment: string }) {
        console.log('Valoración recibida:', event);
        // Aquí se llamaría al servicio para guardar la valoración
        alert(`¡Gracias! Has valorado con ${event.rating} estrellas.`);
    }
}
